...
void AssetManager::installExecutor(const std::string &id, const std::shared_ptr<AssetExecutor> &executor) {
...
void AssetManager::processFileAlternative(std::string file, std::string path, e172::AbstractGraphicsProvider *graphicsProvider) {
    std::string sufix = FileSystem::getSufix(file);
    if(sufix == ".json") {
            ...
            if(assetId != "class" && assetId != "id") {
                ...
                if(it != executors.end()){
                    if(item->isNull()) {
                        ...
                        it->second->proceed(*item, graphicsProvider);
                    }
                } else {
